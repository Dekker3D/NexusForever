...
namespace NexusForever.Game.Entity
{
    public class ContactManager : IContactManager
    {
        ...
        private readonly Dictionary</*guid*/ ulong, IContact> contacts = new();
        private readonly HashSet<IContact> deletedContacts = new();
        private readonly Dictionary</*guid*/ ulong, IContact> pendingRequests = new();
        ...
        public ContactManager(Player player, CharacterModel model)
        {
            ...
            foreach (IContact contact in GlobalContactManager.Instance.GetQueuedRequests(player.CharacterId))
            {
        ...
        public void AcceptFriendRequest(ulong id, bool returnRequest = false)
        {
            ...
            {
                if (GlobalContactManager.Instance.CanBeContact(owner.Session, contactRequest.OwnerId, contactRequest.Type, contacts))
                {
        ...
        {
            if (GlobalContactManager.Instance.CanBeContact(owner.Session, recipientId, ContactType.Rival, contacts))
            {
                if (GetExistingContactByRecipient(recipientId, out IContact existingContact))
        ...
        {
            if (GlobalContactManager.Instance.CanBeContact(owner.Session, recipientId, ContactType.Ignore, contacts))
            {
                if (GetExistingContactByRecipient(recipientId, out IContact existingContact))
                {
        ...
        public void SetPrivateNote(IContact contactToModify, string Note)
        {
            ...
            owner.Session.EnqueueMessageEncrypted(new ServerFriendSetNote
            {
        ...
        /// <summary>
        /// Set a player's <see cref="AccountPresenceState"/>
        /// </summary>
        /// <param name="session">Session of a player to send the update to</param>
        private void SendPersonalStatus()
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendAccountPresenceUpdate
            {
                AccountId = owner.Account.Id,
                Presence = AccountPresenceState.Available
            });
        }
        ...
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendAccountPersonalStatus
            {
                AccountPublicStatus = "",
                AccountNickname = "",
                Presence = AccountPresenceState.Available,
        ...
        {
            var ContactsList = new ServerFriendList
            {
                Contacts = contacts
            };
        ...
        {
            var serverFriendshipAccountList = new ServerFriendAccountList();
        ...
        {
            List<ServerFriendInviteList.InviteData> contactRequestDataList = new();
            ...
            foreach (IContact pendingRequest in contactRequestsToSend)
            {
                ...
                owner.Session.EnqueueMessageEncrypted(new ServerFriendInviteList
                {
                    Invites = contactRequestDataList
                });
            }
        }
        ...
        /// <returns></returns>
        private ServerFriendInviteList.InviteData GetContactRequestData(IContact contactRequest, ICharacter character)
        {
            CalculateExpiryTime(contactRequest, out float expiryTime);

            return new ServerFriendInviteList.InviteData
            {
                InviteId = contactRequest.Id,
                PlayerIdentity = new TargetPlayerIdentity
                {
                    RealmId = RealmContext.Instance.RealmId,
                ...
                Note = contactRequest.InviteMessage,
                Name = character.Name,
        ...
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendshipResult
            {
                Message = "",
                Results = result
            });
        ...
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendInviteRemove
            {
                InviteId = guid
            });
        }
        ...
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendAdd
            {
                Friend = contactData
            });
        }
        ...
        /// <summary>
        /// Send a <see cref="ServerContactsDeleteResult"/> packet for a <see cref="IContact"/> to the player
        /// </summary>
        ...
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendRemove
            {
                FriendshipId = contactId
            });
        }

        /// <summary>
        /// Send a <see cref="ServerContactsUpdateType"/> packet for a <see cref="IContact"/> to the player
        /// </summary>
        /// <param name="contact"></param>
        private void SendContactsUpdateType(IContact contact)
        {
            owner.Session.EnqueueMessageEncrypted(new ServerFriendTypeUpdate
            {
                FriendshipId = contact.Id,
                Type = contact.Type
            });
        }
